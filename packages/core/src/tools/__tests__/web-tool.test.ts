...
describe('webFetchTool', () => {
  ...
  // These tests require network access, which may not be available
  // in sandboxed environments. They validate the tool's core behavior
  // but are skipped when the network is unavailable.
  it.skip('should return error for non-200 response', async () => {
    const result = await execute({ url: 'https://httpbin.org/status/404' });
    expect(result.success).toBe(false);
    expect(result.error).toContain('404');
  });

  it.skip('should fetch a valid URL successfully', async () => {
  ...
  it.skip('should include prompt in output when provided', async () => {
